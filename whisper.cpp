...
static void log_mel_spectrogram_worker_thread(int ith, const std::vector<float> &hann, const float *samples,
    ...
    std::complex<float> *fft_out = fft_alloc<std::complex<float>>(fft_out_size);
    ...
    int mels_to_calculate = n_samples / fft_step;

    for (int i = ith; i < mels_to_calculate; i += n_threads) {
	...
	pocketfft::r2c<float>(fft_shape,
	    ...
	    fft_out[j] = std::norm(fft_out[j]); // This sets the imaginary part to zero
        }

        if (speed_up) {
            // scale down in the frequency domain results in a speed up in the time domain
	    for (int j = 0; j < n_fft; j+=2) {
	      fft_out[j] = (fft_out[2 * j] + fft_out[2 * j + 1]) / (float)2.0;
            }
        }
